...
public class RingerModes extends ContentProvider {

  public static final Uri CONTENT_URI = Uri.parse("content://edu.ncsu.soc.rms/ringer");
  ...
  static {
    ...
    uriMatcher.addURI("edu.ncsu.soc.rms", "ringer", ALERTS);
    uriMatcher.addURI("edu.ncsu.soc.rms", "ringer/#", ALERT_ID);
  }
  ...
  public boolean onCreate() {
    ...
    setRingDB(dbHelper.getWritableDatabase());
    return (getRingDB() == null) ? false : true;
  }
  ...
  public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
    ...
    Cursor c = qb.query(getRingDB(), projection, selection, selectionArgs, null, null, orderBy);
  ...
  public Uri insert(Uri _uri, ContentValues _initialValues) {
    ...
    long rowID = getRingDB().insert(RINGER_TABLE, PLACES, _initialValues);
  ...
  public int delete(Uri uri, String where, String[] whereArgs) {
    ...
    switch (uriMatcher.match(uri)) {
    case ALERTS:
      count = getRingDB().delete(RINGER_TABLE, where, whereArgs);
      break;

    case ALERT_ID:
      String segment = uri.getPathSegments().get(1);
      count = getRingDB().delete(RINGER_TABLE, KEY_ID + "="
          + segment
  ...
  public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
    int count;
    switch (uriMatcher.match(uri)) {
    case ALERTS: count = getRingDB().update(RINGER_TABLE, values, where, whereArgs);
    break;

    case ALERT_ID: String segment = uri.getPathSegments().get(1);
    count = getRingDB().update(RINGER_TABLE, values, KEY_ID
        + "=" + segment
        + (!TextUtils.isEmpty(where) ? " AND ("
            + where + ')' : ""), whereArgs);
    break;
  ...
  public SQLiteDatabase getRingDB() {
	return ringDB;
}

public void setRingDB(SQLiteDatabase ringDB) {
	this.ringDB = ringDB;
}

/** The underlying database */
  
private SQLiteDatabase ringDB;
  private static final String TAG = "RingerProvider";
  private static final String DATABASE_NAME = "ring.db";
  private static final int DATABASE_VERSION = 1;
  private static final String RINGER_TABLE = "ring";
